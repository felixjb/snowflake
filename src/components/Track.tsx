...
import { MILESTONES, categoryColorScale } from '../lib/constants';
...
export function Track({ milestoneByTrack, trackId, handleTrackMilestoneChange }: Props) {
      ...
      <div style={{ display: 'flex' }}>
        ...
        {currentMilestone ? (
          <div style={{ flex: 1 }}>
            ...
            <ul>
              {currentMilestone.signals.map((signal, index) => (
                <li key={index}>{signal}</li>
              ))}
            </ul>
            <h4>Example tasks:</h4>
            <ul>
              {currentMilestone.examples.map((example, index) => (
